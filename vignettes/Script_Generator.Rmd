...
```{r}
res <- script_funs(aet01_1, adam_db = syn_data, args_spec = "args.yaml")
writeLines(res)
```
...
res_funs <- script_funs(aet01_1, adam_db = syn_data, args_spec = "args.yaml")
```
...
res_funs <- script_funs(mng01_1, adam_db = syn_data, args_spec = "args.yaml")
```
...
If saved in a `.R` file, the generated script can be executed.
...
```{r}
res_fun <- script_funs(mng01_1, adam_db = syn_data, args_spec = "args.yaml", details = TRUE)
```
