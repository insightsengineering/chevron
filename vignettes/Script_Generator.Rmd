...
In addition of the embedded `run()` method to create a `tlg`, chevron offers a script-based approach that allow the user to quickly edit a chevron workflow without the need for modifying a `chevron_tlg` object. The script is generated based on the default value of the arguments but can be edit either manually after creation or programmatically with the `dict` argument.

By default, the generated script is printed in the consol but it can also be saved in a file whose address in passed to the `con` argument. 

# Using a chevron-defined object
The script is stored in a temporary R file.
```{r}
...
tmp <- tempfile("my_script", fileext = "R")
script(mng01_1, con = tmp, dict = dict)
```

Print the first 30 lines of the generated script.
```{r, comment = "", echo = FALSE}
cat(readLines(tmp, n = 30), sep = '\n')
```

Source the generated script and inspect the result stored in `final_tlg`.
