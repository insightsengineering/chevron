...
In the output generation, we often need to specify a particular sorting order of a variable at the time of display. In `chevron`, a character variable needs to be factorized with pre-specified levels to display in order. When encountering cases like the following one where `"ARM A"` has an Asian group only while `"ARM B"` has both Asian and White groups, it is not able to produce outputs like the demographic table unless `"RACE"` is factorized to provide access to the same level attribute of the variable `"RACE"` after the arm split.

```{r}
proc_data <- syn_data
proc_data$adsl <- proc_data$adsl %>% mutate(RACE = factor(case_when(
  ARMCD == "ARM A" ~ "ASIAN",
...
Column counts are displayed by default. The generic argument `display_columncounts` controls whether the count of unique number of subjects (N=xxx) will be displayed in the column header or not. `display_columncounts = FASLSE` suppresses the display of N=xxx.
