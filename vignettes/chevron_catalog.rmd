...
For studies with more than one study drug, users need to define the analyses in `adae` and add to the argument `aesi_vars` following the example above. No pre-defined analyses is available at this moment. 
...
2) The template does not include the column of total as default. The 'All Patients' column can be added by the argument `lbl_overall = "All Patients"`.
...
To collapse grade 3-4 with grade 5, this can be achieved by modifying the definition of grade groups in the argument `grade_groups`.
...
2) Currently, the expected name for safety time-to-event dataset is `adaette` rather than GDSR standard naming `adsaftte`.
3) By default, all `adaette` parameters containing 'TTTE' are including in the output.  
4) User needs to subset the parameter(s) of interest from input safety time-to-event dataset using pre-processing if customization is needed.
5) In the input safety time-to-event dataset, the censoring variable `CNSR` was set to 0 for the occurrence of event of interest and set to 1 for censoring.
...
1) The type of the confidence interval for rate can be specified by the argument `conf_type = "exact"`.   
...
2) Currently, the expected name for safety time-to-event dataset is `adaette` rather than GDSR standard naming `adsaftte`.  
3) By default, all `adaette` parameters containing 'TOT' are including in the output.  
4) Users are expected to subset parameters of interest from input analysis dataset in pre-processing if customization is needed.  
5) In the input safety time-to-event dataset, the censoring variable `CNSR` was set to 0 for the occurrence of event of interest and set to 1 for censoring.  
...
1) Study specific continuous variables can be added to the standard demographics and baseline characteristics summary by editing the argument `summaryvars`. To add or remove analyses, you need to list all variables you would like to include in the argument.  
...
2) By default, `ADDV.DVDECOD` provides the category and `ADDV.DVTERM` provides the description.  
3) By default, `addv` has been subset to include records that meet `DVCAT == "MAJOR"`.  
...
3) By default, `addv` has been subset to include records that meet `AEPRELFL == "Y" & DVCAT == "MAJOR"`.  
...
4) By default, the first level or value of `arm_var` (default to ADSL.ARM unless specified) is treated as the reference group. To provide user-defined reference group, please provide the value from the treatment variable to the argument `ref_group`, e.g., `ref_group = "PLACEBO"`.  
...
The section of `Odds Ratio` can be suppressed with the argument `odds_ratio = FALSE`. Note that currently the syntax does not allow excluding the section of difference in overall response rate and its p-value  
...
2) The methods to construct confidence interval are controlled by the argument `methods`. It is a named list with five sub-arguments. Please type `?rspt01` in the console to see more details of available methods to use.

```{r}
proc_data <- log_filter(syn_data, PARAMCD == "BESRSPI", "adrs")

run(rspt01, proc_data,conf_level = 0.90, methods = list(prop_conf_method = "wald", diff_conf_method = "wald",  diff_pval_method = "fisher"))
```

#### **4. Best Overall Response (with stratified analysis)**

1) A stratified analysis can be added by specifying the argument `perform_analysis = "strat"` and provide the stratification variable to the argument `strata` . The argument `strata` is expected if `perform_analysis` is set to include stratified analysis.  The stratification variables are expected in `adrs`.
2) If unstratified and stratified analysis are required, users can use `perform_analysis = c("unstrat",strat")`  
...
#### **5. Best Overall Response (modifying the definition of overall response)**
...
3) By default, the first level or value of `arm_var` (default to ADSL.ARM unless specified) is treated as the reference group. To provide user-defined reference group, please provide the value from the treatment variable to the argument `ref_group`, e.g., `ref_group = "PLACEBO"`.  
4) Unstratified analysis is provided by default. 
5) Survival estimations and difference in survival compared with reference group are both provided by default.
...
To select either survival estimations or difference in survival compared with reference group or both, please specify in the argument `method`.
...
Please use HELP page to find out more details of what options are available in terms of 1) the level of the confidence intervals in the argument `conf_level`, 2) handing of ties in `ties`, 3) type of confidence interval in `conf_type`.  
...
The time point for the "survival at xx months" analysis can be modified by specifying the argument `timepoint`. By default, the survival estimates are summarized at "6 months" and "12 months". 

```{r}
proc_data <- log_filter(syn_data, PARAMCD == "PFS", "adtte")

run(ttet01, proc_data, perform_analysis = "unstrat", timepoint = c(3,6))
```

#### **6. Time-to-event Summary (modifying the p-value method for testing hazard ratio)**

The default p-value method for testing hazard ratio is "log-rank". Alternative methods can be requested by specifying the argument `pval_method` and options include `wald` or `likelihood`. The syntax currently does not allow requesting more than one p-value.
