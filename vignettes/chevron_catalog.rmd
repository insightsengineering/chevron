...
3) By default, all `adaette` parameter codes containing 'TTE' are including in the output. Users are expected to filter the parameter(s) of interest from input safety time-to-event dataset in pre-processing, if not all are needed.
4) In the input safety time-to-event dataset, the censoring variable `CNSR` was set to 0 indicating the occurrence of event of interest and set to 1 for censoring.
...
1) The type of the confidence interval for rate can be specified by the argument `conf_type`. Options include `normal` (default), `normal_log`, `exact`.  
...
2) Currently, the expected name for safety time-to-event dataset is `adaette` rather than GDSR standard naming `adsaftte`.  
3) By default, all `adaette` parameter codes containing 'TOT' are including in the output. Users are expected to filter parameters of interest from input analysis dataset in pre-processing, if not all are needed.  
4) In the input safety time-to-event dataset, the censoring variable `CNSR` was set to 0 indicating the occurrence of event of interest and set to 1 for censoring.  
...
1) The type of the confidence interval for rate can be specified by the argument `conf_type`. Options include `normal` (default), `normal_log`, `exact`, or `byar`.     
2) The confidence interval can be adjusted by the argument `conf_level`.
...
2) Users are expected to filter `addv` to include records that meet `DVCAT == "MAJOR"` in pre-processing.  
...
1) A stratified analysis can be added by specifying the argument `perform_analysis = "strat"` and provide the stratification variable to the argument `strata` . The argument `strata` is expected if `perform_analysis` is set to include stratified analysis.    
2) The stratification variables are expected to be available in `adrs`.  
2) If unstratified and stratified analysis are both required, users can use `perform_analysis = c("unstrat",strat")`  
...
See table below for what each argument controls and the available mthod options:  
```{r table_args, echo=FALSE, results = 'asis'}
tabl <- "
...
See table below the method options for estimates of proportions and the associated statistical methods 
```{r table_prop, echo=FALSE, results = 'asis'}
tabl <- "
...
See table below the method options for estimates of proportion difference and the associated statistical methods 
```{r table_prop_diff, echo=FALSE, results = 'asis'}
tabl <- "
| Method Options  | Statistical Methods                       |
| :--------------:| :----------------------------------------:|
...
See table below the method options for testing proportion difference and the associated statistical methods 
```{r table_test_prop_diff, echo=FALSE, results = 'asis'}
tabl <- "
| Method Options  | Statistical Methods                       |
| :--------------:| :----------------------------------------:|
...
3) Please see the section of `RSPT01` to find out more about the ordering of treatment groups and reference group. 
4) Unstratified analysis is provided by default. 
...
1) The level of the confidence intervals is defined by the argument `conf_level`.    
2) The type of confidence interval is defined in the argument `conf_type`. Options are `"plain"` (default), `"log"`, `"log-log"`.  
3) Handing of ties is specified in the argument `ties`. Options are `"efron"` (default),`"breslow"` or `"exact"`.
...
The time point for the "survival at xx months" analysis can be modified by specifying the argument `timepoint`. By default, the syntax takes `AVAL` from `adtte` in the unit of days and convert to months. The survival estimates are then summarized in month, and the numeric values provided to `timepoint` should be in the unit of month as well. 
