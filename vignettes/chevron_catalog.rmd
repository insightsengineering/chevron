...
#### **3. `auto_pre`: customized pre-processing**
The generic argument `auto_pre` controls whether the default pre-processing step will be performed or not at the execution of `run` function. When modification of the default pre-processing is needed, please include the preprocess function and set `auto_pre = FALSE` to implement the customized `preprocess` function and avoid conflicts potential conflicts.
