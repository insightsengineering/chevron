...
2) Users are expected to pre-process the input analysis data by selecting a time-to-event parameter to be analyzed. If there are more than two treatment groups present in the input analysis data, users are also expected to select only two treatment groups. The example below is based on the time-to-event parameter "Duration of Confirmed Response by Investigator" and treatment groups "Arm A" and "Arm B".
...
To add the interaction term to the model, `interaction = TRUE` needs to be specified.
...
3) Other tie handling methods, i.e., `"efron"` or `"breslow"`, can be specified in the `tie` argument. 
4) Users can also customize the alpha level for the confidence intervals through the `conf_level` argument.
