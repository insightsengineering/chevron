...
By default, the `AGEGR1` variable is used as the age group. Study specific age group variable can be added by editing the parameter `summaryvars`. The age group variable to be categorized needs to be present in both ADSL and ADEX.

```{r}
pre_data <- propagate(syn_data, "adsl", "AGEGR1", "USUBJID")
run(rmpt03, pre_data)
```
...
### **Extent of Exposure by Ethnic Origin for Risk Management Plan (`RMPT05`)**
