...
```{r, echo = FALSE}
lopo_entry <- function(tlgfunc, type = c("table", "listing", "graph"), extra_args = NULL,
                       output_id = NULL, main_title, subtitles = NULL, footnotes = NULL,
                       filters = NULL, filename = NULL) {
    type <- match.arg(type)
    
    if (is.null(output_id))
      output_id <- paste0(substr(type, 1, 1), "_", tolower(substr(tlgfunc, 1, 2)))
    ...
    tibble(tlgfunc, type, extra_args = list(extra_args), output_id, main_title, subtitles = list(subtitles), 
           footnotes = list(footnotes), filters = list(filters), filename)
}

lopo <- rbind(
  lopo_entry(
    tlgfunc = "dmt01_1", type = "table",
    main_title = "Demographics and Baseline Characteristics", filters = "IT"
  ),
  lopo_entry(
    tlgfunc = "aet02_1", type = "table",
...
Note that the first three columns are not needed for stakeholder interactions and planning/project management. However, the GDS template which is currently encoded in the `tlgfunc` is needed.
...
library(purrr)
tbls <- setNames(lapply(transpose(lopo_sel), function(lopoi) {
  
  db <- adam_study_data %>%
    dm_filter_with_labels(lopoi$filters) %>%
    (std_filter(lopoi$tlgfunc))() %>%
    (std_mutate(lopoi$tlgfunc))()
  ...
  tmpl <- null_as_empty_string(list(
    ...
    subtitles = lopoi$subtitles,
    main_footer = lopoi$footnotes
  ))
  ...
  do.call(lopoi$tlgfunc, c(list(adam_db = db, deco = lapply(tmpl, render_whiskers, dict = dict_i))))
