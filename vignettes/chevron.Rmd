...
* A `lyt` list of functions also refereed to as *Layout-list*. (only for `chevron_t` objects)
...
1. If they are of the subclass `chevron_t`, have always as second argument `lyt_ls`, a list of functions returning `Pre-data Table Layout` that are used by the main function to generate the output.
...
### *Layout-list*
*Layout-list* in `chevron` are stored in each `chevron_t` objects as list of functions in the `lyt` slot. These functions must have the formal argument `...`.
...
To create a `chevron_tlg` object from scratch, use one of the provided constructors: 
* `chevron_t()` if the output is a table.
* `chevron_l()` if the output is a listing.
* `chevron_g()` if the output is a graph.

```{r}

my_pipeline <- chevron_t(
  main = aet02_1_main,
