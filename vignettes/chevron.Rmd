...
Each standard output is associated with one  *`chevron_tlg` object*. They contain the following objects:
* A `main` function also refereed to as *TLG-function*.
* A `lyt` list of functions also refereed to as *Layout-function*.
...
1. have always as second argument `lyt_fun`, a list of functions returning `Pre-data Table Layout` that are used by the main function to generate the output.
...
### *Layout-function*
*Layout-function* in `chevron` are stored in each `chevron_tlg` objects as list of functions. These functions must have the formal argument `...`.
...
### *post-processing*
By default, the Post-processing function returns its input or a `NULL` report if the input has no rows but the slot is present to enable user-defined customization. *`post-processing`* function of a `chevron_tlg` object must have at least `tlg` and `...` as formal arguments.
...
In some instances it is useful to customize the `chevron_tlg` object, for instance by changing the pre processing functions. Be aware that you have to think carefully about argument names and compatibility with downstream functions. The `lyt` slot stores a `list of functions` but the constructor and setter method `lyt<-` can handle input of class `function`, `list of functions`, as long as they possess the formal argument `...`, or `Pre-Data Table Layout`. 
...
To create a `chevron_tlg` object from scratch, use the provided constructor:
```{r}
