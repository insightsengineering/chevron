...
* A `lyt` function or list of function also refereed to as *Layout-function*.
...
1. have always as second argument `lyt_fun`, a function or list of function returning `Pre-data Table Layout` that is
used by the main function to generate the output.
...
1. have the `...` argument to facilitate their incorporation in a workflow.

### *post-processing*`
By default, the Post-processing function returns its input but the slot is present to enable user-defined customization.
...
In some instances it is useful to customize the `chevron_tlg` object, for instance by changing the pre processing functions. Be aware that you have to think carefully about argument names and compatibility with downstream functions.
