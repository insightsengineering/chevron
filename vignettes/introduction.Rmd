...
db <- preprocess_data(adam_study_data, "aet02_1")
...
* `preprocess_data` applies the returned function of `std_filter` and `std_mutate` to the data. These functions 
   often return the `identity` function but sometimes contain modifications that do not belong to the ADaM model.
   For example, here:
...
* the functions returned by `std_mutate` and `std_filter` has always the first argument `adam_db` and might have 
  optional arguments, including possibly the `.study` object.
