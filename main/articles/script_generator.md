...
`aet01_custom`` ``<-`` ``aet01`` `[`preprocess`](https://insightsengineering.github.io/chevron/reference/preprocess.md)`(``aet01_custom``)`` ``<-`` ``function``(``adam_db``, ``new_format``, ``...``)`` ``{`` `` `[`reformat`](https://insightsengineering.github.io/dunlin/latest-tag/reference/reformat.html)`(``adam_db``, ``new_format``)`` ``}`` `` ``res_funs`` ``<-`` `[`script_funs`](https://insightsengineering.github.io/chevron/reference/script.md)`(``aet01_custom``, adam_db ``=`` ``"syn_data"``, args ``=`` ``"args_list"``)`
...
[`writeLines`](https://rdrr.io/r/base/writeLines.html)`(``res_funs``)`` ``#> # Edit Preprocessing Function.`` ``#> preprocess(aet01_custom) <- `` ``#> function (adam_db, new_format, ...) `` ``#> {`` ``#> reformat(adam_db, new_format)`` ``#> }`` ``#> `` ``#> # Create TLG`` ``#> tlg_output <- run(object = aet01_custom, adam_db = syn_data, verbose = TRUE, user_args = args_list)`
