test_that("aet05_all can handle NA values", {
  ...
  res <- expect_silent(run(aet05_all, proc_data, dataset = "adaette"))
...
test_that("aet05_all can handle some NA values", {
  ...
  res1 <- expect_silent(run(aet05_all, proc_data, dataset = "adaette"))
  expect_snapshot(cat(export_as_txt(res1, lpp = 100)))

  res2 <- expect_silent(run(aet05_all, proc_data, dataset = "adaette", conf_level = 0.90, conf_type = "byar"))
