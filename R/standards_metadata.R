...
# find them with tools::showNonASCIIfile(file = "R/standards_metadata.R")
...
-	By default, for each medication class, a row summarizing the total number of treatments is displayed.  There is an option to suppress this row (highlighted in grey in mockup).
",
    "Statistics and Calculation Methods" = ":  The percentages are the number of safety-evaluable patients, as given in the column headings.  Count multiple occurrences of the same medication once.",
    "Numeric Precision and Formatting of Statistics" = "Use standard display in the mockup.",
    "Formatting Options" = NULL,
    "Optional Subsetting" = "The user could choose specific classes of medications using selected values of ATC Class Levels 1, 2, 3 or 4 to present for more granular review to support special topic analysis."
  ),
  EXT01 = list(
...
",
  ...
  VST02 = list(
    ...
    "Column Totals" = NULL,
    "Analysis Variables" = "Specify which tests are to be summarized or any tests present in the clinical database that should not be summarized.  If low abnormalities are not possible for a particular test because the lower limit of normal is zero or missing, do not include a row in the table corresponding to low abnormalities for that test. Should accept Vital Sign and ECG datasets.",
    "Statistics and Calculation Methods" = ":  Specify what normal ranges are used to determine the abnormalities (default is to use Roche standard reference ranges).  Table entries provide the number of patients with a during-treatment assessment abnormality in the direction specified.  Option should be made available to include any abnormality regardless of Baseline assessment (VST02_1/EGT02_1), or second option to include only if abnormality not present at Baseline (VST02_2/EGT02_2). Analysis is based on patients with at least one post-baseline value.  The treatment group size (N=nnn in the column heading) is the number of patients in the treatment group. For the first option the cell denominators (/nn) include patients with at least one post-baseline value for the parameter. For the second option the cell denominators (/nn) include patients with at least one post-baseline value for the parameter and without the abnormality at baseline. This can cause nn to be less than N for this option. Patients with missing baseline values are assumed normal for purposes of determining abnormality. All available data are used (i.e., scheduled and unscheduled visits, repeat tests) in identifying abnormalities.  Baseline is defined by the user.  User can specify whether values from samples collected more than a specified time interval (e.g., 4 weeks) after the last dose of study drug should be excluded.  Ensure that the footnote matches your specification.
If an abnormality is possible for a test but not observed in the database, include that test and abnormality with zeros in the numerator for each treatment group.",
    "Numeric Precision and Formatting of Statistics" = "As given in the mock table",
    "Formatting Options" = "Allow the option for portrait or landscape output orientation",
