...
#' @param time (`string`) time variable in a Cox proportional hazards regression model.
#' @param event (`string`) event variable in a Cox proportional hazards regression model.
...
coxt02_1_main <- function(adam_db,
  ...
  assert_colnames(dbsel$adtte, c(arm_var, time, event, covariates, strata))
...
#' @export
#'
coxt02_1_pre <- function(adam_db, arm_var = "ARM", time = "AVAL", event = "CNSR",
                         covariates = c("SEX", "RACE", "AAGE"), strata = NULL, ...) {
  coxt02_1_check(adam_db,
    arm_var = arm_var, time = time, event = event,
...
#' @export
#'
coxt02_1_check <- function(adam_db, arm_var, time, event, covariates, strata,
